...
in
{
  ...
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      # new window
      bind-key Enter new-window -c "#{pane_current_pat}"
      ...
      bind-key C-Space run-shell -b "${pkgs.tmuxPlugins.tmux-fzf}/share/tmux-plugins/tmux-fzf/scripts/window.sh switch"
      bind-key -n C-t run-shell "fzf-sesh"
